...
const ResultModal = ({selectedAgents}) => {
    ...
    for(let i = 0; i < selectLen; i++) {
        if (selectedAgents[i].standards.length < lowest) lowest = selectedAgents[i].standards.length;
        if (selectedAgents[i].standards.length == lowest) standKey = selectedAgents[i].standards;
    }
          ...
          <Button variant="primary" onClick={handleShow}>
            Submit
              ...
              <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body>Your exclusion list
                    <ul>
